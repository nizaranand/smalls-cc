...
function wpul_option_page(){
	add_menu_page(__('UserLogin', 'wp-userlogin'),__('UserLogin', 'wp-userlogin'),10,__FILE__,'wpul_userlogin_options_page');	
	add_submenu_page(__FILE__,__('CSS Editor', 'wp-userlogin'),__('CSS Editor', 'wp-userlogin'),'manage_options','wpul_style_options','wpul_style_editor');	
}
...
function wpul_initial_db($args){
	...
	$autoload = "yes";
	$deprecated = " ";
	$boxvalue = array('CHECKED','CHECKED','CHECKED','CHECKED','CHECKED','CHECKED','CHECKED','CHECKED','CHECKED','CHECKED');
...
function wpul_user_permissions($args){
	...
	if($check['dashboard'] == "CHECKED" && $val == "0"){
		$link[0] = '<li><a href="'.$wp_url.'/wp-admin/">'.__('Dashboard').'</a></li>';
	}
	if($check['newpost'] == "CHECKED" && $val == "1"){
$link[1]= '<li><a href="'.$wp_url.'/wp-admin/post-new.php">'.__('New Post').'</a></li>';
	}
	if($check['editpost'] == "CHECKED" && $val == "2"){
$link[2]= '<li><a href="'.$wp_url.'/wp-admin/edit.php">'.__('Edit Posts').'</a></li>';
	}
	if($check['managetheme'] == "CHECKED" && $val == "3"){
$link[3]= '<li><a href="'.$wp_url.'/wp-admin/themes.php">'.__('Manage Theme').'</a></li>';
	}
	if($check['options'] == "CHECKED" && $val == "4"){
$link[4]= '<li><a href="'.$wp_url.'/wp-admin/options-general.php">'.__('Options').'</a></li>';
	}
	if($check['users'] == "CHECKED" && $val == "5"){
$link[5]= '<li><a href="'.get_bloginfo('wpurl').'/wp-admin/users.php">'.__('Users').'</a></li>';
	}
	if($check['profile'] == "CHECKED" && $val == "6"){
$link[6]= '<li><a href="'.$wp_url.'/wp-admin/profile.php">'.__('Your Profile').'</a></li>'.PHP_EOL.
'<li><a href="'.$wp_url.'/?author='.$user_ID.'">'.__('View Your Profile','wp-userlogin').'</a></li>';
	}
	if($check['logout'] == "CHECKED" && $val == "7"){
$link[7]='<li><a href="'.wp_logout_url(get_bloginfo('url').'/'.$check['redirect_out']).'">'.__('Logout').'</a></li>';	
	}        
 	}
...
#// END set display based on selected fields & user permission

#// BEGIN build sidebar widget
function wpul_widget_userlogin($args) {
 ...
 //> If user ID is present print out the options
	if (is_user_logged_in() ) {
		$title =$option['set_log'];	
	
            if ( $user_level >= 8 ){
                    $options = wpul_user_permissions('0,1,2,3,4,5,6,7');
            }
            
            elseif( $user_level >= 5 && $user_level < 8 ){
                    $options = wpul_user_permissions('0,1,2,5,6,7');
            }
                    
            elseif( $user_level >= 1 && $user_level < 5 ){
                    $options = wpul_user_permissions('0,1,6,7');
            }
            
            elseif($user_level == 0 ){
                    $options = wpul_user_permissions('0,6,7');
            }
        }		

 //> If no user ID is present show registration link and login form
	else {
		$title = $option['set_nonlog'];


			$redir = get_bloginfo('url').'/'.$option['redirect'];
		$form = "<form name=\"loginform\" id=\"loginform\" action=\"".wp_login_url($redir) ."\" method=\"post\">
                                    <label for=\"log\">".__('Username')."<br /><input type=\"text\" name=\"log\" id=\"log\" value=\"".  wp_specialchars(stripslashes($user_login), 1) ."\" /></label><br />
				<label for=\"pwd\">".__('Password')."<br /><input type=\"password\" name=\"pwd\" id=\"pwd\" /></label><br />\n";

 //> If WP-OpenID plugin is found active show OpenID login field
 //> WP-OpenID plugin can be found at (http://wordpress.org/extend/plugins/openid/)
 ...
 //> Build Submit button, Remember Me box, and links for registations and forgotten password
	...
	if(get_option('users_can_register')) {
		$register =  "\n<a href=\"". get_bloginfo('wpurl') ."/wp-login.php?action=register\">".__('Register')."</a> | ";
	}
	...
	$recover = "\n<a href=\"".wp_lostpassword_url( $_SERVER['REQUEST_URI'] )."\">".__('Lost your password?')."</a>";
	
	$submit = $remember.$register.$recover;

 //> Cobble the form, OpenID option, and submission together
$options = $form.$openid.$submit;
	}

 //> Build widget based on login status and user role
		echo $before_title
		. $title
		. $after_title
		. $options;
...
function wpul_widget() {
	global $user_ID, $user_identity, $user_level;
	$args = array('name' => '','id' => '', 'description' => '','before_widget' => '<li>','after_widget' =>'</li>','before_title' =>'<h2>','after_title' => '</h2>');
	wpul_widget_userlogin($args);	
}
function wpul_style() {
