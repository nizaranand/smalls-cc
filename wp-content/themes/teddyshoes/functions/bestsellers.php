...
$sql						=
	"select prodid, count(prodid) as prodnum
	from		wp_wpsc_cart_contents
	group by prodid
	order by prodnum
	desc LIMIT 5";
$best_sellers_list	=	$wpdb	->	get_results	(	$sql,
																ARRAY_A
															);
//var_dump($best_sellers_list);			

foreach	((	array	)	$best_sellers_list	as	$i	=>	$best_sellers	)
{	$sql	=
		"SELECT list.id,list.name,list.price,image.image,list.special,list.special_price
	  FROM ".$wpdb->prefix."wpsc_product_list AS list
	  LEFT JOIN ".$wpdb->prefix."wpsc_product_images AS image
	  ON list.image=image.id
	  WHERE list.active = '1'
	  AND list.id=" . $best_sellers['prodid'];
...
if	(	$product	[	'price'	]	-	$product[	'special_price'	]	>	0
...
if	(	$product[	'image'	]):
	if	(	$options[	'themelayout'	] == 'boxed'	):
		$output	=	"<img src='"
						...
						.	"/timthumb.php?src="
						.	WPSC_THUMBNAIL_URL.$product[	'image'	]
						.	"&w=174' title='"
						.	$product[	'name'	]
						.	"' alt='"
						.	$product[	'name'	]
...
echo
	...
	.	wpsc_product_url(	$product[	'id'	])
	.	'">'
	.	$output
	.	'</a>	</div>';

echo
	"<div class	=	'producttext'>	<h3>	<a	href	=	'"
	.	wpsc_product_url(	$product[	'id'	])
	.	"'>"
